...
The wizard ships a real dark palette and picks it up on its own. Three sources
are consulted, in increasing priority:

1. **The operating system**, via `prefers-color-scheme`.
2. **Your app's own switch** — an ancestor carrying `[data-theme="dark"]` or a
   `.dark` class. That covers Tailwind, next-themes, Docusaurus and Fumadocs
   without configuration.
3. **The `darkMode` prop**, which pins it explicitly.
...
Through v1 the stylesheet contained no `prefers-color-scheme` rules at all.
Dark mode existed only as inline styles you had to enumerate through
`customDarkModeColor`, and because they were inline they overrode any CSS you
wrote. `customDarkModeColor` still works, but theme tokens are the better tool.
